...
# 스케일링 방식 MinMaxScaler #
# 데이터를 0과 1 사이의 값으로 스케일링하는 방법 #
...
for stock_name in selected_stock_names:
    ...
    # 데이터 정규화 및 LSTM 입력 데이터 생성
    scaler = MinMaxScaler()
    scaled_data = scaler.fit_transform(
        df[['close_price', 'stock_volume', 'open_price', 'high_price', 'low_price', 'stock_cd']])
    ...
    model.add(LSTM(units=50, return_sequences=True,
                   input_shape=(sequence_length, 6)))  # 6개의 속성을 가지고 있음 (종가, 거래량, 시가, 고가, 저가, stock_cd)
    model.add(LSTM(units=50))
    model.add(Dense(units=6))  # 종가, 거래량, 시가, 고가, 저가
