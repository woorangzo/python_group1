...
price_volume_difference = []  # 주가 차이 및 거래량을 저장할 리스트
...
for i in range(len(df_subset)):  # 각 주식에 대해 반복합니다.
    ...
    volume_today = df_subset.iloc[i]['stock_volume']

    difference = predicted_tomorrow - predicted_today
    price_volume_difference.append((i, difference, volume_today))  # 인덱스, 차이, 거래량을 튜플 형태로 리스트에 추가합니다

# 주가 상승 폭 및 거래량을 기준으로 주식을 정렬합니다.
sorted_stocks = sorted(price_volume_difference, key=lambda x: (x[1], x[2]), reverse=True)[:5]

# 상위 5개 주식의 인덱스와 차이를 가져옵니다.
top_5_indices = [stock[0] for stock in sorted_stocks]
top_5_differences = [stock[1] for stock in sorted_stocks]

# DataFrame에 SQL 결과 저장
df_subset = pd.read_sql(sql_query, conn)

# 상위 5개 주식의 정보를 출력하거나 사용합니다.
top_5_stock_info = df_subset.iloc[top_5_indices][['stock_cd', 'stock_nm', 'stock_dt', 'close_price', 'stock_volume']]
print("내일 예측된 주가 상승이 가장 높은 상위 5개 주식 (거래량 고려):")
